<template>
  <div class="vld-parent">
    ...
    <v-expansion-panels>
      <v-expansion-panel v-for="(user, index) in userList" :key="index">
        <v-expansion-panel-header v-slot="{ open }">
          <v-row no-gutters>
            <v-col cols="4">{{ user.username }}</v-col>
        ...
        <v-expansion-panel-content>
          <v-row no-gutters>
            ...
            <v-col cols="2">ID : {{ user.id }}</v-col>

            <v-col cols="2">Occupation : {{ user.occupation }}</v-col>

            <v-col cols="2">AGE : {{ user.age }}</v-col>

            <v-col cols="2">
              Gender :
              <v-icon v-if="user.gender === 'M'" color="blue">mdi-human-male</v-icon>
          ...
          <v-card-actions v-if="!user.is_staff">
            ...
            <v-btn text color="primary">Modify</v-btn>
