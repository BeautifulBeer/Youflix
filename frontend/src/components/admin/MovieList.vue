<template>
  <div class="vld-parent">
    <Loading loader="dots" color="#E50914" :active.sync="isLoading" :is-full-page="fullPage" />
    ...
    <v-container class="pa-4 text-center">
      <v-row class="fill-height" align="center" justify="center">
        <template v-for="(movie, i) in movieList">
          <v-col :key="i" cols="12" md="2">
            <v-hover v-slot:default="{ hover }">
              <v-card
                ...
                @click="clickMovie(i)"
              >
                <v-img :src="movie.backdrop_path | staticPath" height="225px">
                  <v-card-title class="title white--text">
                    <v-row class="fill-height flex-column" justify="space-between">
                      <p class="mt-4 subheading text-left">{{ movie.title }}</p>

                      <p
                        class="caption font-weight-medium font-italic text-left"
                      >Average Rating : {{ movie.average_rating }}</p>
                    </v-row>
                  </v-card-title>
                </v-img>
              </v-card>
      ...
      <v-dialog v-if="!isLoading" v-model="dialog" width="700">
        <v-card>
          <v-card-title
            v-if="!modifyFlag"
            id="originTitle"
            ...
            primary-title
          >{{ movieList[clickedIndex].title }}</v-card-title>

          <div v-else>
            <br />
            <v-text-field v-model="movieList[clickedIndex].title" label="Input Title" outlined />
          </div>
          ...
          >
            ...
            <iframe
              ...
              alt="require('../../assets/img/commingsoon.jpg')"
            />
          </v-card-text>
          ...
          <v-card-text
            ...
            class="text--primary"
            style="margin: 10px;"
          >{{ movieList[clickedIndex].overview == "" ? "There is no plot information." : movieList[clickedIndex].overview }}</v-card-text>
          <v-textarea
            v-else
          ...
          <v-card-text v-if="!modifyFlag">Genre : {{ movieList[clickedIndex].genres_array }}</v-card-text>
          <v-combobox
            v-else
          ...
          <v-card-text v-if="!modifyFlag">Runtime : {{ movieList[clickedIndex].runtime }}</v-card-text>
          <v-text-field
            v-else
          ...
          <v-card-actions class="text-center">
            <v-row>
              <v-col v-if="!modifyFlag">
                <v-btn color="error" text @click="modifyFlag = true">MODIFY</v-btn>
              </v-col>
              <v-col v-else>
                <v-btn color="error" text @click="modifyMovie()">SAVE</v-btn>
              </v-col>
              <v-col v-if="!modifyFlag">
...
export default {
  ...
  filters: {
    staticPath(backdrop_path) {
      if (
        backdrop_path === "https://image.tmdb.org/t/p/originalNone" ||
        backdrop_path === undefined ||
        backdrop_path === ""
      )
        return require("../../assets/img/commingsoon.jpg");
  ...
  methods: {
    async getAllMovies() {
      ...
      await API.getAllMovies({
        ...
        if (this.movieList.length != 0)
          this.movieList.push.apply(this.movieList, ret.data);
        else this.movieList = ret.data;
    ...
    async modifyMovie() {
      ...
      await API.modifyMovie(data).then(ret => {
        this.isLoading = false;
